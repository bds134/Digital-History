...
# Download or sync this course's Git repo
# Complete the path below to the 'data' folder
knitr::opts_knit$set(root.dir = "D:/Google Drive/git-projects/Digital-History/modules/10-r-mapping/data")
```
# R/Leaflet Tutorial
...
# Download or sync this course's Git repo
# Complete the path below to the 'data' folder
setwd("D:/Google Drive/git-projects/Digital-History/modules/10-r-mapping/data")
```
...
```{r}
leaflet() %>%
  leaflet::addPolygons(data = south, color = "red", weight = 1, fill = TRUE, fillOpacity = .6)
```
...
leaflet() %>%
  leaflet::addPolygons(
    ...
    # tell R to use the color palette 
    # we created based on the population 1960 variable
    fillColor = ~ pal_fun(PO60),
    # these next two lines change the opacity of the layer
    # and smooth out the polygons a bit - you can keep these
    # settings or play around with them if you want to.
    # Both range between 0 and 1
    fillOpacity = 0.8,
    smoothFactor = 0.5
  )
```
...
leaflet() %>%
  ...
  addLegend("bottomright", # location of legend
    pal = pal_fun,
    # palette function - tells R what colors to include in the legend
...
p_popup3 <- paste0(
  "<strong>County: </strong>", south$NAME, ", ", south$STATE_NAME,
